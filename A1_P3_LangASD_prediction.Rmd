...
Answer:
The model from last assignment was used in the first exercise: lmer(CHI_MLU~VISIT+Diagnosis+types_CHI+verbalIQ+(1+VISIT|SUBJ)
The model has a root mean squared error of 0.31, when performed  on the train data
And a root mean squared error of 0.42, when tested on a new dataset
...
lmer(CHI_MLU~VISIT+Diagnosis+types_CHI+verbalIQ+(1+VISIT|SUBJ), data = dtrain_temp)
With a mean rmse of 0.31, and of 0.57 when tested on a new dataset.
```{r}
...
#Get results
rmse_train
mean(rmse_train) #0.3230275

rmse_test
mean(rmse_test) #0.6204807

#THE LOOP FOR THE BEST MODEL
for (i in Folds) {
  #Make a test dataset with one fold
  dtest_temp = subset(traind, SUBJ %in% i)
  #Make a training dataset with all other folds
...
#Get results
rmse_train
mean(rmse_train) #0.3171907

rmse_test
mean(rmse_test) #0.5652438
```
...
#Create Bernie dataset. He is Kid number  (we know it from the original test dataset, where subjects have names)
...
#The difference is very small, he is scoring slightly more than an average typically developing kid, so based on this, I would conclude that Bernie is typically developing kid. 
```
