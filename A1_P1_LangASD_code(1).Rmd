...
```{r}
d_token <- read.csv("~/Assignemnt1/token_train.csv")
d_demo <- read.csv("~/Assignemnt1/demo_train(1).csv")
d_lu <- read.csv("~/Assignemnt1/LU_train.csv")
```
...
```{r}
library(plyr)
d_demo= rename(d_demo, c(Child.ID="SUBJ", Visit = "VISIT"))
```
...
```{r}
library(tidyverse)
d_dn <- select(d_demo, SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw)
d_ln <- select(d_lu,SUBJ, VISIT, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
d_tn <-select(d_token,SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
```{r}
d_vis=data[data$VISIT==1,]
d_vis=plyr::rename(d_vis,c(ADOS="ADOSx", MullenRaw="nonVerbalIQ", ExpressiveLangRaw="verbalIQ"))
newdata=merge(data,
              select(d_vis, SUBJ,ADOSx, nonVerbalIQ, verbalIQ), by="SUBJ") #we have extra columns

newdata=subset(newdata, select = -c(ADOS,MullenRaw, ExpressiveLangRaw)) #dataset without NAs
...
#filtering data 1
cmlurows=filter(newdata, newdata$CHI_MLU > 2.7) #getting rows
cmlunames=unique(cmlurows$SUBJ) #getting names from matching rows
str(cmlunames)

#filtering data 2
cmlurows2=filter(newdata, newdata$CHI_MLU < 1.5& newdata$VISIT==1) #getting rows
cmlunames2=unique(cmlurows2$SUBJ) 
str(cmlunames2)

#filtering data 3
completevisits=filter(newdata, newdata$VISIT== 6) #find last visit rows
completenames=unique(completevisits$SUBJ) #create a list of who made it to the sixth visit
completenames=as.factor(completenames) #as factor

everyone=unique(cleandata$SUBJ) #create a list with all names i nit
everyone=as.factor(everyone)
library(dplyr)
setdiff(everyone,completenames) #find subjects, that don't appear in both lists, meaning didn't complete all trials
...
mostwords=arrange(completevisits,desc(types_CHI)) #completevisits - only vists 6; arrange list descending by types_CHI - amount of words
mostwords[1,] #that's the kid with the most words on the 6th visit - subject 26

#arranging 2
firstvisits=filter(newdata, newdata$VISIT== 1) #find first visit rows
leastwords=arrange(firstvisits,types_CHI) #arrange list by types_CHI
leastwords[1,] #that's the kid with the least words on 1st visit - subject 57
...
```{r}
asdalldata <- filter(cleandata, cleandata$Diagnosis=="ASD")
asd<- select(asdalldata,SUBJ,Diagnosis, CHI_MLU, tokens_CHI)#select coumns needed
asd2<- select(asdalldata,SUBJ,Diagnosis, CHI_MLU,CHI_MLU,CHI_MLU, tokens_CHI) #NOTHING HAPPENED.. IS IT THE POINT?
...
# 3
#By averaing al the data we lose the opportunity to track progress if the kids throughout trial. There is also no indication of how accurate data is after transformation. No indicators of variability or other traits of distributions. The average may not be a accurate description at all."
