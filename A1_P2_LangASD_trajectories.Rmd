...
#over time
data$VISIT<- as.factor(data$VISIT)
ggplot(data, aes(VISIT, MOT_MLU))+geom_boxplot()
...
```{r}
themodel <- lmer(CHI_MLU~VISIT+Diagnosis+types_CHI+verbalIQ+(1+VISIT|SUBJ), data=data) #model by sofie and helene
summary(themodel) #output
r.squaredGLMM(themodel)
